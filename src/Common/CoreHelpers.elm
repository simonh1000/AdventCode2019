...
type alias GraphContext node nodeKey =
    { key : node -> nodeKey
    , children : node -> List node
    }


visit : GraphContext a comparable -> a -> ( Set comparable, List a ) -> ( Set comparable, List a )
visit ({ key, children } as ctxt) node (( marked, acc ) as state) =
    if Set.member (key node) marked then
        state

    else
        let
            ( newMarked, newResult ) =
                List.foldl (visit ctxt) ( Set.insert (key node) marked, acc ) (children node)
        in
        ( newMarked, node :: newResult )


{-| Same as topoWalk, but the caller can provide a function to produce a
comparable value from a node
-}
topoWalkBy : GraphContext a comparable -> a -> List a
topoWalkBy ctxt start =
    visit ctxt start ( Set.empty, [] )
        |> Tuple.second
        |> List.reverse
...
-}
topoWalk : (comparable -> List comparable) -> comparable -> List comparable
topoWalk children =
    topoWalkBy { key = identity, children = children }


topoSortBy : GraphContext a comparable -> List a -> List a
topoSortBy ctxt unsorted =
    List.foldl (visit ctxt) ( Set.empty, [] ) unsorted
        |> Tuple.second
        |> List.reverse


topoSort : (comparable -> List comparable) -> List comparable -> List comparable
topoSort children =
    topoSortBy { key = identity, children = children }
